...
cedula_doc varchar(40) NOT NULL UNIQUE, /*referencia para asignar la cedula del doc, es de tipo varchar, no puede estar vacio, es unica y no puede ser modificable*/
...
telefono_doc int NOT NULL,	/*referencia para asignar telefono principal de contacto con el doc, es de tipo varchar, no puede estar vacio y puede ser modificable*/
email_doc varchar(90) NOT NULL,		/*referencia para asignar email  del doc, es de tipo text, no puede estar vacio y puede ser modificable*/
pass_doc varchar(8), /*Contraseña de doc*/
primary key (id_doc))ENGINE=InnoDB;

DELIMITER //
CREATE TRIGGER doc_to_login AFTER INSERT ON doces FOR EACH ROW
BEGIN
